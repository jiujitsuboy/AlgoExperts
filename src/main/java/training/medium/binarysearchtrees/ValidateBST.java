...
//		testBST();
	...
	public static void testTreeNode() {
		...
		System.out.println(String.format("Is Valid: %b", isValidBST(node10)));
	 ...
	 * Complexity: Time O(n)
	 * 			   Space O(d) where d is the length of the largest branch. It is not O(log(n)), because unlike the binary search, we here need to go down
	 * all the branch until we can said it is valid, so we are going to have on the stack as much as node that branch will have
	 * @param tree
	 * @return
	 */
...
//		System.out.println(node);
		...
		if (node == null) {
			...
			valid = validateSubTree(node.getLeftChild(), minValue, node.getValue())
					&& validateSubTree(node.getRightChild(), node.getValue(), maxValue);
